...
<template>
    <section class="flex flex-col gap-5 w-full h-fit justify-center items-center min-h-screen">
        <form class="flex flex-col gap-10 mb-5 px-5 py-5 rounded-md justify-center items-center">
            ...
            <button class="border-1 text-white/50 hover:text-white px-5 py-2 text-[20px]  border-white w-1/2 " @click.prevent="dropPassword()">Войти</button>
        </form>
        <NuxtLink class="text-[20px] text-white/50 hover:text-white" to="auth">Вернуться!</NuxtLink>
    </section>
</template>
